...
export default function StudentGradesPage() {
  ...
  const filteredGrades = allGrades.filter((grade: any) => {
    if (selectedPeriod !== "all" && grade.period?.id !== selectedPeriod)
      return false;
    if (selectedSubject !== "all" && grade.subject?.id !== selectedSubject)
      return false;
    return true;
  });
  ...
  const subjectAverages = subjects
    .map((subject: any) => {
      const subjectGrades = sortedGrades.filter(
        (g: any) => g.subject?.id === subject.id
      );
          ...
          <Card className="relative overflow-hidden border-blue-200 bg-gradient-to-br from-blue-50 to-white">
            ...
            <CardContent className="relative px-2 sm:px-4 md:px-6 pb-1.5 sm:pb-2 md:pb-6">
              <div className="text-lg sm:text-xl md:text-3xl font-bold text-blue-700 leading-tight">
                ...
                <span className="text-xs sm:text-sm md:text-lg font-normal text-blue-600">
                  /20
                </span>
              </div>
                ...
                {sortedGrades.length > 0
                  ? (() => {
                      const best = sortedGrades.reduce((max: any, g: any) => {
                        ...
                        const maxScore =
                          max.maxScore > 0
                            ? (max.score / max.maxScore) * 20
                            : 0;
                        return score > maxScore ? g : max;
                      }, sortedGrades[0]);
                      const bestScore =
                        best.maxScore > 0
                          ? ((best.score / best.maxScore) * 20).toFixed(2)
                          : "--";
                      return bestScore;
                    })()
                  : "--"}
                <span className="text-xs sm:text-sm md:text-lg font-normal text-green-600">
                  /20
                </span>
              </div>
              <p className="mt-0.5 text-[9px] sm:text-[10px] md:text-xs text-gray-600">
            ...
            {subjectAverages.map((item: any) => (
              ...
              >
                <CardHeader className="flex flex-row items-center justify-between px-3 sm:px-6 pt-3 sm:pt-6 pb-2 sm:pb-4">
                  ...
                  <Badge
                    variant={
                      parseFloat(item.average) >= 10 ? "default" : "destructive"
                    }
                    className="text-xs"
                  >
                      ...
                      >
                        ...
                        <div className="text-right ml-2">
                          <div className="flex items-baseline gap-1 sm:gap-2">
                            <span
                              className={`text-base sm:text-xl md:text-2xl font-bold ${getGradeColor(
                                grade.score,
                                grade.maxScore
                              )}`}
                            >
                  ...
                  <div className="h-1.5 bg-gray-100 rounded-full overflow-hidden">
                    <div
                      ...
                      style={{
                        width: `${
                          sortedGrades.length > 0
                            ? (gradeDistribution.good / sortedGrades.length) *
                              100
                            : 0
                        }%`,
                      }}
                ...
                <CardContent className="px-3 sm:px-6 pb-3 sm:pb-6">
                  <div className="space-y-2">
                    {reportCards.slice(0, 3).map((rc: any) => (
                      <button
                        ...
                        onClick={() =>
                          router.push(
                            `/parent/children/${studentId}/report-cards/${rc.id}`
                          )
                        }
                  ...
                  {reportCards.length > 3 && (
                    <Button
                      ...
                      onClick={() =>
                        router.push(`/parent/children/${studentId}/report-cards`)
                      }
                    >
