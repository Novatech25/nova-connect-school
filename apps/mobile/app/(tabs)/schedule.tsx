...
import { useAuth, useScheduleSlotsByTeacher, useUpcomingSessions } from "@novaconnect/data";
...
export default function SchedulePage() {
  ...
  // Récupérer l'EDT du prof
  const { data: slots, isLoading, refetch } = useScheduleSlotsByTeacher(user?.id || "");
  const { data: upcomingSessions } = useUpcomingSessions(user?.id || "", "teacher", 5);
  ...
  if (isLoading) {
    return (
      <View style={styles.container}>
        <Text style={styles.loadingText}>Chargement de votre emploi du temps...</Text>
      </View>
    );
  }
    ...
    >
      <View style={styles.header}>
        ...
        <Text style={styles.subtitle}>
          {slots?.length || 0} créneaux cette semaine
        </Text>
      </View>
        ...
        <View style={styles.section}>
          ...
          {upcomingSessions.map((session) => (
            <View key={session.id} style={styles.sessionCard}>
              ...
              <Text style={styles.sessionClass}>{session.class?.name}</Text>
            ...
            .map((slot) => (
              <View key={slot.id} style={styles.slotCard}>
                ...
                <View style={styles.slotDetails}>
                  ...
                  <Text style={styles.slotClass}>{slot.class?.name}</Text>
