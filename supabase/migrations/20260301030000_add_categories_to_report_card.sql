-- Migration: Add Category Name to Report Card Data
-- Description: Modifies generate_report_card_data to include subject category (UE) 
-- Created: 2026-03-01
...
BEGIN
  ...
  -- Calcul structuré des moyennes avec Category (UE)
  WITH raw_subject_grades AS (
    ...
    GROUP BY s.id, s.name, s.coefficient, sc.name
    HAVING COUNT(g.id) > 0
  ),
