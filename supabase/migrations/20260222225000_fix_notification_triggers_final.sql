-- Migration: Fix payroll notification triggers to match the ACTUAL notifications table schema
-- Real columns: id, user_id, school_id, type, title, message, data, read_at, created_at
-- Removed: body (use message), channels, priority, sent_at (all non-existent)

-- Fix: Notify teacher when lesson hours are validated
...
BEGIN
  IF NEW.status = 'validated' AND (OLD.status IS NULL OR OLD.status != 'validated') THEN
    ...
    INSERT INTO notifications (school_id, user_id, type, title, message, data, read_at, created_at)
    VALUES (
      school_id_val,
      NEW.teacher_id,
      'hours_validated',
...
-- Fix: Notify teacher when payment is recorded
