...
BEGIN
  ...
  subject_level_grades AS (
    SELECT
      es.student_id,
      g.subject_id,
      -- Si le niveau est collège/lycée, le coefficient de la matière est dicté par le coefficient de l'examen s'il existe, sinon on garde le coef de la matière (fallback)
      CASE 
        WHEN v_level_type IN ('middle_school', 'high_school') THEN
           COALESCE(MAX(CASE WHEN g.grade_type IN ('exam', 'composition') THEN g.coefficient END), s.coefficient)
        ELSE
           s.coefficient
      END as subject_coef,
      -- Devoirs
      SUM(CASE WHEN g.grade_type IN ('homework', 'quiz', 'project', 'participation') THEN (g.score / g.max_score) * 20 ELSE 0 END) as hw_score_sum,
      SUM(CASE WHEN g.grade_type IN ('homework', 'quiz', 'project', 'participation') THEN 1 ELSE 0 END) as hw_coef_sum,
      -- Examen (toujours ramené sur 20 pour le calcul de Moyenne Comp)
      ...
      -- Fallback pour Primaire/Université
  ...
  student_subject_averages AS (
    SELECT
      ...
      CASE
        WHEN v_level_type IN ('middle_school', 'high_school') THEN
          CASE
            WHEN slg.hw_coef_sum > 0 AND slg.ex_coef_sum > 0 THEN ((slg.hw_score_sum / slg.hw_coef_sum) + ((slg.ex_score_sum / slg.ex_coef_sum) * 2.0)) / 3.0
            WHEN slg.ex_coef_sum > 0 THEN (slg.ex_score_sum / slg.ex_coef_sum) -- Examen uniquement
    ...
    WHERE slg.total_coef_sum > 0 OR (slg.hw_coef_sum > 0 OR slg.ex_coef_sum > 0)
  ),
  student_averages AS (
    SELECT
      ssa.student_id,
...
BEGIN
  ...
  WITH raw_subject_grades AS (
    SELECT
      ...
      -- Si le niveau est collège/lycée, le coefficient principal de la matière devient le coef de l'examen s'il existe
      CASE 
        WHEN v_level_type IN ('middle_school', 'high_school') THEN
           COALESCE(MAX(CASE WHEN g.grade_type IN ('exam', 'composition') THEN g.coefficient END), s.coefficient)
        ELSE
           s.coefficient
      END as total_coefficient,
      SUM(CASE WHEN g.grade_type IN ('homework', 'quiz', 'project', 'participation') THEN (g.score / g.max_score) * 20 ELSE 0 END) as hw_score_sum,
      SUM(CASE WHEN g.grade_type IN ('homework', 'quiz', 'project', 'participation') THEN 1 ELSE 0 END) as hw_coef_sum,
      SUM(CASE WHEN g.grade_type IN ('exam', 'composition') THEN (g.score / g.max_score) * 20 * g.coefficient ELSE 0 END) as ex_score_sum,
      SUM(CASE WHEN g.grade_type IN ('exam', 'composition') THEN g.coefficient ELSE 0 END) as ex_coef_sum,
    ...
    SELECT
      ...
      (
        CASE
          WHEN v_level_type IN ('middle_school', 'high_school') THEN
            CASE
              WHEN hw_coef_sum > 0 AND ex_coef_sum > 0 THEN ((hw_score_sum / hw_coef_sum) + ((ex_score_sum / ex_coef_sum) * 2.0)) / 3.0
              WHEN ex_coef_sum > 0 THEN (ex_score_sum / ex_coef_sum) -- Examen uniquement
